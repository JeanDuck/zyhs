...
public class Internet {
     ...
     * @return
     */
    public static String changeorder(String urlStr,String name,int num,int state,String sign)
    {
        com.alibaba.fastjson.JSONObject corder = new com.alibaba.fastjson.JSONObject();
        corder.put("name",name);
        corder.put("num",num);
        corder.put("state",state);
        corder.put("sign",sign);
        try {
            URL url=new URL(urlStr);
            HttpURLConnection connect=(HttpURLConnection)url.openConnection();
            connect.setDoInput(true);
            ...
            connect.setRequestProperty("Content-Type", "application/json;charset=UTF-8");
            OutputStream outputStream = connect.getOutputStream();
            outputStream.write(corder.toJSONString().getBytes());
            int response = connect.getResponseCode();
            if (response== HttpURLConnection.HTTP_OK)
            {
                System.out.println(response);
